...
function SlackSection() {
  const gold = "#c9a84c";
  const [status, setStatus] = useState({ connected: false, hasWebhook: false, hasBot: false, oauthUrl: null });
  ...
  useEffect(() => {
    fetch("/api/slack/connect").then(r => r.json()).then(d => setStatus(d)).catch(() => null);
    if (typeof window !== "undefined") {
      const p = new URLSearchParams(window.location.search);
      if (p.get("slack")) setSlackStatus(p.get("slack"));
    }
  }, []);
  ...
  async function handleSendCheckin(type) {
    ...
    try {
      ...
      setCheckinResult(data.error ? { error: data.error } : { ok: "Check-in sent to Slack!" });
    } catch (e) { setCheckinResult({ error: e.message }); }
    setCheckinLoading(false);
  }

  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: status.connected ? "1px solid rgba(16,185,129,0.2)" : "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", padding: "24px", marginBottom: "12px" }}>
      {slackStatus === "connected" && (
        <div style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: "10px", padding: "12px 16px", marginBottom: "16px" }}>
          <p style={{ fontSize: "13px", color: "#6ee7b7", margin: 0 }}>✓ Slack connected! PsychFlo will now send safety alerts and check-in prompts to your workspace.</p>
        </div>
      )}
      {slackStatus === "cancelled" && (
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", padding: "12px 16px", marginBottom: "16px" }}>
          <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.5)", margin: 0 }}>Slack connection cancelled.</p>
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          ...
          <div>
            ...
            <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", margin: 0 }}>Safety alerts + daily check-in prompts sent directly to your team.</p>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
          {status.connected && <span style={{ fontSize: "10px", fontWeight: "700", background: "rgba(16,185,129,0.15)", color: "#6ee7b7", padding: "3px 10px", borderRadius: "999px" }}>CONNECTED</span>}
          {status.connected && (
            <button onClick={() => setOpen(o => !o)}
              style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.6)", padding: "9px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", cursor: "pointer" }}>
              {open ? "Close" : "Manage"}
            </button>
          )}
          {status.oauthUrl ? (
            <a href={status.oauthUrl}
              style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: status.connected ? "rgba(255,255,255,0.06)" : "#4A154B", border: status.connected ? "1px solid rgba(255,255,255,0.1)" : "none", color: "#fff", padding: "9px 18px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: "pointer", textDecoration: "none" }}>
              <span style={{ fontSize: "16px" }}>💬</span>
              {status.connected ? "Reconnect to Slack" : "Add to Slack"}
            </a>
          ) : !status.connected ? (
            <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", fontStyle: "italic" }}>Setup required — see below</span>
          ) : null}
        </div>
      </div>

      {!status.oauthUrl && !status.connected && (
        <div style={{ marginTop: "20px", background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "10px", padding: "16px" }}>
          <p style={{ fontSize: "12px", fontWeight: "700", color: gold, margin: "0 0 8px", letterSpacing: "0.04em" }}>ONE-TIME SETUP (2 MINUTES)</p>
          <ol style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", margin: 0, paddingLeft: "18px", lineHeight: "2" }}>
            <li>Go to <a href="https://api.slack.com/apps" target="_blank" rel="noreferrer" style={{ color: gold }}>api.slack.com/apps</a> → Create New App → From manifest</li>
            <li>Paste the manifest from <strong style={{ color: "rgba(255,255,255,0.7)" }}>slack-app-manifest.json</strong> in this project</li>
            <li>Copy the <strong style={{ color: "rgba(255,255,255,0.7)" }}>Client ID</strong> and <strong style={{ color: "rgba(255,255,255,0.7)" }}>Client Secret</strong></li>
            <li>Add to Vercel: <code style={{ background: "rgba(255,255,255,0.08)", padding: "1px 6px", borderRadius: "4px" }}>SLACK_CLIENT_ID</code> and <code style={{ background: "rgba(255,255,255,0.08)", padding: "1px 6px", borderRadius: "4px" }}>SLACK_CLIENT_SECRET</code></li>
            <li>Redeploy — the "Add to Slack" button will appear here</li>
          </ol>
        </div>
      )}
      ...
      {open && status.connected && (
        <div style={{ marginTop: "24px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <p style={{ fontSize: "12px", fontWeight: "700", color: "rgba(255,255,255,0.3)", marginBottom: "14px", letterSpacing: "0.06em" }}>SEND CHECK-IN PROMPT NOW</p>
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            {[{ type: "standup", label: "Send standup check-in" }, { type: "pulse", label: "Send pulse check-in" }].map(({ type, label }) => (
              <button key={type} onClick={() => handleSendCheckin(type)} disabled={checkinLoading}
                style={{ background: `linear-gradient(135deg,${gold},#f0d080)`, color: "#0f172a", border: "none", padding: "10px 20px", borderRadius: "8px", fontSize: "13px", fontWeight: "700", cursor: checkinLoading ? "not-allowed" : "pointer", opacity: checkinLoading ? 0.5 : 1 }}>
                {checkinLoading ? "Sending…" : label}
              </button>
            ))}
          </div>
          {checkinResult && (
            <p style={{ fontSize: "12px", color: checkinResult.error ? "#f87171" : "#6ee7b7", marginTop: "10px" }}>
              {checkinResult.error || checkinResult.ok}
            </p>
          )}
          <div style={{ marginTop: "16px", fontSize: "11px", color: "rgba(255,255,255,0.3)", lineHeight: "1.7" }}>
            <strong style={{ color: "rgba(255,255,255,0.5)" }}>Automatic alerts:</strong> PsychFlo sends a Slack alert whenever a standup or onboarding check-in returns a low safety score or flags are detected. No action needed.
          </div>
        </div>
      )}
    </div>
